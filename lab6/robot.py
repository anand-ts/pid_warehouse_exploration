...
class Robot(object):
    ...
    def pickup_marker(self, grid, dis=1):
        ...
        if len(self.markers_found_or_picked) != len(
            set(self.markers_found_or_picked).union(set(marker_list))
        ):
            self.markers_found_or_picked = list(
                set(self.markers_found_or_picked).union(set(marker_list))
            )
            fname = grid.fname
            print(
                f"{fname} found {len(self.markers_found_or_picked)}/{grid.LANDMARKS_TOTAL} markers"
            )
        return
    ...
    def move_diff_drive(self, grid, vl, vr, dt):
        ...
        if m == math.inf:
            m, c = find_line(
                (self.__x + 0.01, self.__y), (self.__x + dx, self.__y + dy)
            )

        x_range = [self.__x + 0.1 * i for i in range(1, math.floor(dx / 0.1))]
        x_range = [self.__x] + x_range + [self.__x + dx]
        for xi in x_range:
            yi = m * xi + c
            if not grid.is_free(math.floor(xi), math.floor(yi)):
                raise Exception(
                    f"grid ({math.floor(xi)}, {math.floor(yi)}) isn't free error"
                )
        self.__x += dx
        self.__y += dy
